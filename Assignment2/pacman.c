...
int main(int argc, char *argv[100]) {
    ...
    // initialisation of the struct pointer and it's field to 
    // default values
    outputfile = malloc(sizeof(output_t));
    outputfile -> max_depth = 0;
    outputfile -> total_expanded = 0;
    outputfile -> total_generated = 0;
    outputfile -> time = 0;
    ...
    if (argc > 2 ) {
        ...
        if( strcmp(argv[3],"avg")==0 ){
            propagation = avg;
            outputfile -> propagation = "avg";//set the string to avg
        }
        else if( strcmp(argv[3],"max")==0 ){
            propagation = max;
            outputfile -> propagation = "max"; //set the string to max
        }
        else{
            print_usage();
            return 0;
...
void ExitProgram(const char *message) {
    ...
    printf("Final Score: %d\n",Points); // Display Score
    
    FILE* fp = fopen("outputfile.txt","w"); // Open the outfile.txt in writing mode
    
    fprintf(fp,"%s\n%d\n%d\n%d\n%d\n%6f\n%d\n%d\n", outputfile -> propagation,budget,
    outputfile -> max_depth, outputfile -> total_expanded,outputfile ->
    total_generated, outputfile -> time, (int)( outputfile -> total_expanded/ outputfile -> time),Points); // write out propagation, budget, max_depth, total_expanded,
               // outputfile, expanded_nodes per second, score
    
    fclose(fp);                     //Close outputfile
...
void MainLoop() {
    ...
    do {
        ...
        if(ai_run){
             ...
             */
            move_t selected_move = get_next_move( current_state, budget, propagation, ai_stats, outputfile);
